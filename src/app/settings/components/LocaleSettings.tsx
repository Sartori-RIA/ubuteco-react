...
export function LocaleSettings() {
  ...
  if (!canManage || !organization?.id) {
    return (
      <p className="text-sm text-muted">
        Only organization admins can change locale, currency, and timezone.
      </p>
    );
  }
  ...
  const submitSettings = async (payload: FormState) => {
    ...
    try {
      const updated = await organizationsService.update(organization.id, payload);
      if (!user) return;
      ...
      if (error instanceof ApiError) {
        ...
        setErrors(["Could not save regional settings."]);
      }
    } finally {
      setSaving(false);
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <p className="text-xs text-muted">
          Changing currency affects new products and orders only. Existing orders keep their recorded
          currency.
        </p>
      ...
      <ConfirmDialog
        ...
        message="This updates how new menu items and orders are priced. Existing orders and line items are not converted."
